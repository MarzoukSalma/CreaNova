...
import { Heart, Upload, Plus, X, Camera, Quote, Smile, Frown, Meh, Sun, Moon, Coffee, Star, Filter, Grid, List } from 'lucide-react';
...
const GalleryPage = () => {
  ...
  useEffect(() => {
    const sampleInspirations = [
      ...
      },
      {
        ...
        author: "Proverbe",
        mood: 'sad',
        image: 'https://images.unsplash.com/photo-1502780402662-acc01917286e?w=400&h=300&fit=crop',
  ...
  useEffect(() => {
    const filtered = inspirations.filter(inspiration => inspiration.mood === selectedMood);
  ...
  const saveInspiration = () => {
    ...
    const newInspiration = {
      ...
      image: uploadData.imagePreview || 'https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400&h=300&fit=crop',
      author: 'Vous',
      isUserCreated: true
    };
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50">
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-gray-600">Trouvez l'inspiration selon votre humeur du moment</p>
        </div>
        ...
        <div className="flex justify-between items-center mb-8">
          <div className="flex items-center gap-4">
            <div className={`px-4 py-2 bg-gradient-to-r ${moods[selectedMood].color} text-white rounded-full font-semibold flex items-center gap-2`}>
              {moods[selectedMood].icon}
              Mode {moods[selectedMood].name}
            </div>
            <span className="text-gray-600">
              {filteredInspirations.length} inspiration{filteredInspirations.length !== 1 ? 's' : ''}
            </span>
          </div>
          ...
          {filteredInspirations.map((inspiration) => (
            ...
            >
              <div className={`relative ${viewMode === 'list' ? 'w-1/3' : 'h-64'}`}>
                ...
                />
                <div className={`absolute top-4 left-4 px-3 py-1 bg-gradient-to-r ${moods[inspiration.mood].color} text-white rounded-full text-sm font-semibold flex items-center gap-1`}>
              ...
              <div className={`p-6 ${viewMode === 'list' ? 'w-2/3 flex flex-col justify-center' : ''}`}>
                <h3 className="font-bold text-xl mb-3 text-gray-800">{inspiration.title}</h3>
        ...
        {filteredInspirations.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-gray-600 mb-6">
              Soyez le premier à ajouter une inspiration pour l'humeur "{moods[selectedMood].name}" !
            </p>
            <button
              onClick={() => setIsUploadModalOpen(true)}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-gray-800">Nouvelle Inspiration</h2>
              <button
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:border-purple-400 transition-colors">
                  {uploadData.imagePreview ? (
                    <div className="relative">
                      ...
                      />
                      <button
                        onClick={() => setUploadData(prev => ({ ...prev, image: null, imagePreview: '' }))}
                    ...
                    <div>
                      ...
                      <p className="text-gray-600 mb-4">Cliquez pour ajouter une image</p>
                      <input
                        type="file"
                        accept="image/*"
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {Object.entries(moods).map(([key, mood]) => (
                    <button
                      key={key}
                      onClick={() => setUploadData(prev => ({ ...prev, mood: key }))}
